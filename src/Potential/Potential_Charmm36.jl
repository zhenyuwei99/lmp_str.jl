...
"""
    function Potential_Charmm36_Unit_create(para_info, complete_mode="normal", para_complete=0)
...
# Output
para_mass = [Potential_Charmm36_Unit_create(info) for info in mass_para_info]
para_pair = [Potential_Charmm36_Unit_create(info, "copy", 4) for info in pair_para_info]
for pair in para_pair
    pair.para[[2, 4]] *= 2 / 2^(1/6)
end
para_bond = [Potential_Charmm36_Unit_create(info) for info in bond_para_info]
para_angle = [Potential_Charmm36_Unit_create(info, "zero", 4) for info in angle_para_info]
para_dihedral = [Potential_Charmm36_Unit_create(info, "one") for info in dihedral_para_info]
para_improper = [Potential_Charmm36_Unit_create(info) for info in improper_para_info]
